...
# define RT_MDL_TYPE      RT_MODEL_two_MSD_system_T
#endif
#define QUOTE1(name) #name
#define QUOTE(name) QUOTE1(name)    /* need to expand name    */
#ifndef FMI2_FUNCTION_PREFIX
#define FMI2_FUNCTION_PREFIX two_MSD_system_
#endif
/*==========*
 * Includes *
 *==========*/
...
void fmu_LogOutput() {
    ...
    modelData->rOutput[1] = *(modelData->rList[2]);
    modelData->rOutput[2] = *(modelData->rList[3]);
    modelData->rOutput[3] = *(modelData->rList[4]);
}
void fmu_RestoreOutput() {
    FMUModelData *modelData = (FMUModelData *)_instance;
    *(modelData->rList[1]) = modelData->rOutput[0];
    *(modelData->rList[2]) = modelData->rOutput[1];
    *(modelData->rList[3]) = modelData->rOutput[2];
    *(modelData->rList[4]) = modelData->rOutput[3];
}
...
fmi2Component fmi2Instantiate(fmi2String instanceName, fmi2Type fmuType, fmi2String fmuGUID,
    ...
    modelData->rList = (real_T**)calloc(12, sizeof(real_T*));
    if (!modelData->rList) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Memory allocation error.");
        return NULL;
    }
    modelData->rOutput = (real_T*)calloc(4, sizeof(real_T));
    if (!modelData->rOutput) {
        functions->logger(functions->componentEnvironment, instanceName, fmi2Error, "error",
                "fmi2Instantiate: Memory allocation error.");
        return NULL;
    }
    modelData->rList[0] = &(two_MSD_system_U.InputForce);
    modelData->rList[1] = &(two_MSD_system_Y.Positionx1);
    modelData->rList[2] = &(two_MSD_system_Y.Positionx2);
    modelData->rList[3] = &(two_MSD_system_Y.Velocityv1);
    modelData->rList[4] = &(two_MSD_system_Y.Velocityv2);
    modelData->rList[5] = &(modelData->time);
    modelData->rList[6] = &(two_MSD_system_P.b1);
    modelData->rList[7] = &(two_MSD_system_P.b2);
    modelData->rList[8] = &(two_MSD_system_P.k1);
    modelData->rList[9] = &(two_MSD_system_P.k2);
    modelData->rList[10] = &(two_MSD_system_P.m1);
    modelData->rList[11] = &(two_MSD_system_P.m2);
    fmu_syncToBuffer();
...
fmi2Status fmi2DoStep(fmi2Component c, fmi2Real currentCommunicationPoint,
    ...
    steps = (int)(communicationStepSize / (double)6 + 0.5);    
    if (modelData->loggingOn) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
            fmi2OK, "info", "CommunicationStepSize=%.16g, LocalSolverStepSize=%.16g", communicationStepSize, (double)6);
    }
    if (communicationStepSize < 0 || fabs(communicationStepSize - steps * (double)6) > 1.0E-6 * (double)6) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
                    fmi2Error, "error", "Stepsize must be non-negative and divisible by %.16g", (double)6);
        returnStatus = fmi2Error;
    } else if (fabs(modelData->time - currentCommunicationPoint) > 1.0E-6 * (double)6) {
        modelData->functions->logger(modelData->functions->componentEnvironment, modelData->instanceName, 
